...
| `MCP_MAX_REQUEST_BODY_BYTES` | No | `104857600` (100 MB) | Maximum `/mcp` JSON request-body size in bytes (ceiling: 1 GiB). Raise it when `load_image` needs a larger `docker save` tar — the base64 MCP payload is ~33% larger than the raw tar. A malformed or non-positive value is ignored with a startup warning and the default is used. Passed through in `docker-compose.yml`, so setting it in `.env` reaches the container |
